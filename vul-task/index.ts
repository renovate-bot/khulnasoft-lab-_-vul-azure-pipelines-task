...
const latestVulVersion = "v0.38.2"
const tmpPath = "/tmp/"

async function run() {

    console.log("Preparing output location...")
    const outputPath = tmpPath + "vul-results-" + Math.random() + ".json";
    ...
    const assurancePath = tmpPath + "vul-assurance-" + Math.random() + ".json";
    ...
    console.log("Running Vul...")
    let result = runner.execSync();
    if (result.code === 0) {
        task.setResult(task.TaskResult.Succeeded, "No problems found.")
    } else {
        task.setResult(task.TaskResult.Failed, "Failed: Vul detected problems.")
    }

    if(hasAccount) {
        console.log("Publishing JSON assurance results...")
        task.addAttachment("ASSURANCE_RESULT", "vul-assurance-" + Math.random() + ".json", assurancePath)
    }

    console.log("Publishing JSON results...")
    task.addAttachment("JSON_RESULT", "vul" +  Math.random() + ".json", outputPath)
    console.log("Done!");
}
...
async function createRunner(docker: boolean, loginDockerConfig: boolean): Promise<ToolRunner> {
    ...
    if (!docker) {
        console.log("Run requested using local Vul binary...")
        const vulPath = await installVul(version)
        return task.tool(vulPath);
    }
    ...
    runner.line("khulnasoft/vul:" + stripV(version))
    return runner
}
...
async function installVul(version: string): Promise<string> {

    console.log("Finding correct Vul version to install...")
...
async function getArtifactURL(version: string): Promise<string> {
    if(version === "latest") {
        version = latestVulVersion
    }
    console.log("Required Vul version is " + version)
    let arch = ""
    switch (os.arch()) {
        case "arm":
            arch = "ARM"
    ...
    // e.g. vul_0.29.1_Linux-ARM.tar.gz
    let artifact: string = util.format("vul_%s_Linux-%s.tar.gz", stripV(version), arch);
    return util.format("https://github.com/khulnasoft-lab/vul/releases/download/%s/%s", version, artifact);
}
