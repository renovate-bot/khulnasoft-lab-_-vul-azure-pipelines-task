...
import {Report, AssuranceReport} from './vul'
...
export class App extends React.Component<AppProps, AppState> {
    ...
    async check() {
        ...
        timeline.records.forEach(function (record: TimelineRecord) {
            if (record.type == "Task" && record.task !== null && record.task.name == "vul") {
        ...
        if (attachments.length === 0) {
            this.setState({error: "No attachments found: cannot load results. Did Vul run properly?"})
            return
        }
